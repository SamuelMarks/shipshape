...
- Diff viewer for refit changes (Monaco editor)
...
| **`shipshape_ui`** | Angular 21 frontend with `ngx-monaco-editor` for diff reviews. |
...
### Server & Web UI
*Requires PostgreSQL and a GitLab instance (SaaS or Self-Hosted).*
...
```bash
docker build -f shipshape-server/Dockerfile -t shipshape-server .
...
```bash
docker build -f shipshape-server/Dockerfile \
